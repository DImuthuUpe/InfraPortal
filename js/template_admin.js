...
function mainMenuCollapse(img) {
	...
	if (nextLi.style.display == "none") {
		// remember the state
		document.cookie = img.id + "=visible;path=/;expires="
		...
		img.src = baseAt+"/images/up-arrow.gif";

	} else {
		// remember the state
		document.cookie = img.id + "=none;path=/;expires="
				+ cookie_date.toGMTString();
		var ver = getInternetExplorerVersion();
		...
		img.src = baseAt+"/images/down-arrow.gif";
...
function setMainMenus() {
	...
	for (var i = 0; i < els.length; i++) {
		...
		if (nextLiState != null) {
			if (get_cookie('menuPanelType') == null
					|| get_cookie('menuPanelType') == "main") { // Set the menu
				// to main
		...
		if (nextLiState == "none") {
			els[i].src = baseAt+"/images/down-arrow.gif";
		} else {
			els[i].src = baseAt+"/images/up-arrow.gif";
		}
	}
}
...
YAHOO.util.Event
		.onAvailable(
				...
				function() {
					...
					if (/Firefox[\/\s](\d+\.\d+)/.test(navigator.userAgent)) { // test
						// for
						// Firefox/x.x
						// or
						// Firefox
						...
						// remaining
						// digits);
						var ffversion = new Number(RegExp.$1); // capture x.x
						// portion and
						// store as a
						// number
					...
					var menuSliderTxt1 = "<span>Admin</span>";
					var menuSliderTxt2 = "<span>User</span>";
//					var menuSliderTxt3 = "<span>Configure</span>";
//					var menuSliderTxt4 = "<span>Tools</span>";
//					var menuSliderTxt5 = "<span>Extensions</span>";
					if (getInternetExplorerVersion() != -1) {
						menuSliderTxt1 = '<span class="ie">Admin</span>';
						menuSliderTxt2 = '<span class="ie">User</span>';
...
//						menuSliderTxt5 = '<span class="ie">Extensions</span>';
					...
					var menuSlider0 = document
							.getElementById('menu-panel-button0');
					var menuSlider1 = document
							.getElementById('menu-panel-button1');
					
					var menuSlider2 = document
							.getElementById('menu-panel-button2');
//					var menuSlider3 = document
//							.getElementById('menu-panel-button3');
//					var menuSlider4 = document
//							.getElementById('menu-panel-button4');
//					var menuSlider5 = document
//							.getElementById('menu-panel-button5');
...
//					}
					...
					if (get_cookie('menuPanel') == null
							|| get_cookie('menuPanel') == "visible") {
					...
					if (get_cookie('menuPanelType') == null
							|| get_cookie('menuPanelType') == "main") { // Set
						// the
						// menu
						// to
						// main
						// show the main section
						var elmsX = YAHOO.util.Selector
								.query("#menu-table ul.main > li");
...
//					}
					YAHOO.util.Event
							.on(
									menuSlider0,
									"click",
									function(e) { // arrow click
										if (menuPanel.style.display == "") {
											menuPanel.style.display = "none";
											YAHOO.util.Dom
													.removeClass(menuSlider0,
															'showToHidden');
											YAHOO.util.Dom
													.addClass(menuSlider0,
															'hiddenToShow');
											document.cookie = "menuPanel=none;path=/;expires="
													+ cookie_date.toGMTString();
											...
											YAHOO.util.Dom
													.addClass(menuSlider0,
															'showToHidden');
											document.cookie = "menuPanel=visible;path=/;expires="
													+ cookie_date.toGMTString();
					...
					YAHOO.util.Event
							.on(
									menuSlider1,
									"click",
									function(e) { // Handle click for main
										// menu

										menuPanel.style.display = "";
										YAHOO.util.Dom.removeClass(menuSlider0,
												'hiddenToShow');
										YAHOO.util.Dom.addClass(menuSlider0,
												'showToHidden');
										document.cookie = "menuPanel=visible;path=/;expires="
												+ cookie_date.toGMTString();
										document.cookie = "menuPanelType=main;path=/;expires="
												+ cookie_date.toGMTString();
										onMenuSlide.fire('visible');
					...
					YAHOO.util.Event
							.on(
									menuSlider2,
									"click",
									function(e) { // Handle click for Monitor
										// menu
										menuPanel.style.display = "";
										YAHOO.util.Dom.removeClass(menuSlider0,
												'hiddenToShow');
										...
										document.cookie = "menuPanel=visible;path=/;expires="
												+ cookie_date.toGMTString();
										document.cookie = "menuPanelType=monitor;path=/;expires="
												+ cookie_date.toGMTString();
										onMenuSlide.fire('visible');
